...
<html>
<head>
    <title> Concerts Search Results </title>
    ...
    <div class="ui page grid">
      ...
      <?php
          ...
          if (!empty($_POST["searchterm"])) {
              ...
              $results = $db->query("SELECT * FROM concerts WHERE $type LIKE '%$term%'");
              /* check if the result if empty*/
              if ($results->rowCount() == 0)
              {
                  echo "<p>No concert found with $term.</p>";
              }
              else 
              {
